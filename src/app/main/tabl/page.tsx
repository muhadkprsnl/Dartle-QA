...
const SprintDashboard = () => {
    ...
    const processFetchedData = (
        ...
        const dev = rawData
            ...
            .filter((item) => filterBySprint(item, "Sprint") && filterByDateRange(item))
            // .filter(
            //     (item) =>
            ...
            // .filter((item) => {
            //     const sprintMatch = filterBySprint(item, "Sprint");
            //     const dateMatch = filterByDateRange(item);
            //     if (!sprintMatch || !dateMatch) {
            //         console.log("Filtered out:", item); // ✅ TEMP: debug
            //     }
            //     return sprintMatch && dateMatch;
            // })
            ...
            .map((item) => ({
                ...
                closeDate: item.closeDate   // ✅ add this
            }))
        ...
        setProdData(prod);
        console.log("Filtering by:", searchSprint, dateRange);
        // await fetchData(); // Re-fetch entire dataset from backend
    ...
    //     setDevData: React.Dispatch<React.SetStateAction<SprintData[]>>,
    //     setProdData: React.Dispatch<React.SetStateAction<SprintData[]>>,
    //     setEditingId: React.Dispatch<React.SetStateAction<string | null>>,
    //     setEditingEnv: React.Dispatch<React.SetStateAction<"dev" | "prod" | null>>
    // ) => {
    //     e.preventDefault();

    //     // 🛠 Fix: convert date fields to ISO
    //     const payload = {
    //         ...editFormData,
    //         dueDate: editFormData.dueDate ? new Date(editFormData.dueDate).toISOString() : undefined,
    //         closeDate: editFormData.closeDate ? new Date(editFormData.closeDate).toISOString() : undefined,
    //     };

    //     try {
    //         const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/reports/${editFormData._id}`, {
    //             method: "PUT",
    //             headers: {
    ...
    //             alert("Failed to update report: " + errorText);
    //             return;
    //         }
    ...
    //         if (editingEnv === "dev") {
    //             setDevData((prev) => prev.map((item) => (item._id === updated._id ? updated : item)));
    //         } else {
    //             setProdData((prev) => prev.map((item) => (item._id === updated._id ? updated : item)));
    //         }
    ...
    const handleEditFormSubmit = async (
        ...
        try {
            ...
            const frontendFormat: SprintData = {
                _id: updatedReport._id,
                sprint: updatedReport.sprint,
                version: updatedReport.version,
                totalTestCases: updatedReport.totalTestCases,
                totalBugs: updatedReport.totalBugs,
                developers: [
                    {
                        name: updatedReport.developer1,
                        passed: updatedReport.d1Passed,
                        failed: updatedReport.d1Failed,
                    },
                    {
                        name: updatedReport.developer2,
                        passed: updatedReport.d2Passed,
                        failed: updatedReport.d2Failed,
                    },
                ].filter(dev => dev.name),
                environment: updatedReport.environment,
                dueDate: updatedReport.dueDate,
                closeDate: updatedReport.closeDate,
            };

            // Update the appropriate state based on environment
            if (editingEnv === "dev") {
                setDevData(prev =>
                    prev.map(item => item._id === frontendFormat._id ? frontendFormat : item)
                );
            } else {
                setProdData(prev =>
                    prev.map(item => item._id === frontendFormat._id ? frontendFormat : item)
    ...
    const refreshData = async () => {
        ...
        try {
            ...
            if (!res.ok) throw new Error("Failed to fetch data");
            const data = await res.json();
            setOriginalRawData(data);
            processFetchedData(data, searchSprint, dateRange);
        } catch (err) {
            setError("Failed to load data.");
            console.error(err);
        } finally {
    ...
    // Call this after successful updates
    // refreshData();
